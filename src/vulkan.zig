...
fn IsExtensionAvailable(properties: []const vk.ExtensionProperties, extension: []const u8) bool {
    std.debug.print("Checking for extension: {s}\n", .{extension});
    for (0..properties.len) |i| {
        if (std.mem.eql(u8, &properties[i].extension_name, extension)) return true;
    } else return false;
}
...
pub const Vulkan = struct {
    ...
    pub fn init(allocator: Allocator, app_name: [*:0]const u8, window: *c.GLFWwindow) !Vulkan {
    ...
    pub fn deviceName(self: *const Vulkan) []const u8 {
    ...
    pub fn allocate(self: Vulkan, requirements: vk.MemoryRequirements, flags: vk.MemoryPropertyFlags) !vk.DeviceMemory {
