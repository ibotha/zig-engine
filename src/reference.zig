...
var g_VmaAllocator: c.VmaAllocator = null;
...
fn vkCreateInstance(info: [*c]const c.VkInstanceCreateInfo, allocator: [*c]const c.VkAllocationCallbacks, instance: [*c]c.VkInstance) c.VkResult {
    ...
    _ = allocator;
    return func(info, null, instance);
}
...
fn key_callback(window: ?*c.GLFWwindow, key: c_int, scancode: c_int, pressed: c_int, mods: c_int) callconv(std.builtin.CallingConvention.c) void {
    _ = window;
    _ = scancode;
    if (key == c.GLFW_KEY_R and pressed == 1 and (mods & c.GLFW_MOD_SHIFT) != 0) {
        g_SwapChainRebuild = true;
    }
}

fn framebuffer_resize_callback(window: ?*c.GLFWwindow, width: c_int, height: c_int) callconv(std.builtin.CallingConvention.c) void {
    _ = width;
    _ = height;
    _ = window;
    g_SwapChainRebuild = true;
...
fn IsLayerAvailable(layers: []const c.VkLayerProperties, layer: [*:0]const u8) bool {
    std.debug.print("Checking for layer: {s}\n", .{layer});
...
fn SetupVulkan(allocator: std.mem.Allocator, instance_extensions: *std.ArrayList([*:0]const u8)) !void {
    ...
    app_info.pApplicationName = "MusicPlatformer";
    app_info.applicationVersion = g_ApiVersion;
    app_info.pEngineName = "IsardsEngine";
    ...
    var vma_funcs: c.VmaVulkanFunctions = .{};
    vma_funcs.vkGetInstanceProcAddr = get_vulkan_instance_func(c.PFN_vkGetInstanceProcAddr, g_Instance, "vkGetInstanceProcAddr").?;
    ...
    vma_funcs.vkGetDeviceProcAddr = get_vulkan_instance_func(c.PFN_vkGetDeviceProcAddr, g_Instance, "vkGetDeviceProcAddr").?;
    check_vk_result(err);
    ...
    const vma_create_info = c.VmaAllocatorCreateInfo{
        .physicalDevice = g_PhysicalDevice,
        .instance = g_Instance,
        .pAllocationCallbacks = g_Allocator,
        .pVulkanFunctions = &vma_funcs,
        .flags = c.VMA_ALLOCATOR_CREATE_EXT_MEMORY_BUDGET_BIT,
        .vulkanApiVersion = g_ApiVersion,
        .device = g_Device,
    };
    err = c.vmaCreateAllocator(&vma_create_info, &g_VmaAllocator);
    check_vk_result(err);
...
fn CleanupVulkan() void {
    ...
    c.vmaDestroyAllocator(g_VmaAllocator);
    g_Allocator = null;
...
pub fn main() !void {
    ...
    // _ = c.glfwSetWindowSizeCallback(window, window_resize_callback);
