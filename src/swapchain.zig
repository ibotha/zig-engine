...
pub const Swapchain = struct {
    ...
    pub fn init(gc: *const Vulkan, allocator: Allocator, extent: vk.Extent2D) !Swapchain {
        return try initRecycle(gc, allocator, extent, .null_handle);
    }

    pub fn initRecycle(gc: *const Vulkan, allocator: Allocator, extent: vk.Extent2D, old_handle: vk.SwapchainKHR) !Swapchain {
    ...
    fn deinitExceptSwapchain(self: Swapchain) void {
        for (self.swap_images) |si| si.deinit(self.v);
        self.allocator.free(self.swap_images);
        self.v.dev.destroySemaphore(self.next_image_acquired, null);
    }

    pub fn waitForAllFences(self: Swapchain) !void {
        for (self.swap_images) |si| si.waitForFence(self.v) catch {};
    }
    ...
    pub fn deinit(self: Swapchain) void {
        ...
        self.v.dev.destroySwapchainKHR(self.handle, null);
    }
    ...
    pub fn present(
        ...
        try current.waitForFence(self.v);
        try self.v.dev.resetFences(1, @ptrCast(&current.frame_fence));

        // Step 2: Submit the command buffer
        const wait_stage = [_]vk.PipelineStageFlags{.{ .top_of_pipe_bit = true }};
        const rsemaphores = [_]vk.Semaphore{
            current.render_finished,
        };
        const buffers = [_]vk.CommandBuffer{
            cmdbuf.?,
        };
        try self.v.dev.queueSubmit(self.v.graphics_queue.handle, 1, &[_]vk.SubmitInfo{
            .{
                ...
                .signal_semaphore_count = rsemaphores.len,
                .p_signal_semaphores = @ptrCast(&rsemaphores),
            },
        }, current.frame_fence);
        ...
        _ = try self.v.dev.queuePresentKHR(self.v.present_queue.handle, &.{
        ...
        const result = try self.v.dev.acquireNextImageKHR(
            self.handle,
            std.math.maxInt(u64),
...
const SwapImage = struct {
    ...
    fn init(gc: *const Vulkan, image: vk.Image, format: vk.Format) !SwapImage {
    ...
    fn deinit(self: SwapImage, gc: *const Vulkan) void {
    ...
    fn waitForFence(self: SwapImage, gc: *const Vulkan) !void {
        _ = try gc.dev.waitForFences(1, @ptrCast(&self.frame_fence), .true, std.math.maxInt(u64));
    }
};

fn initSwapchainImages(gc: *const Vulkan, swapchain: vk.SwapchainKHR, format: vk.Format, allocator: Allocator) ![]SwapImage {
...
fn findSurfaceFormat(gc: *const Vulkan, allocator: Allocator) !vk.SurfaceFormatKHR {
...
fn findPresentMode(gc: *const Vulkan, allocator: Allocator) !vk.PresentModeKHR {
